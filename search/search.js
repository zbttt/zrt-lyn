...
Page({
  ...
  onsearchinput:function(event){
    ...
    const that=this
    wx.request({
      url: 'http://101.200.158.165:8082/superadmin/searchGoodsByName?name='+a,
      method:'GET',
      success(res){
            var arrayup=[]
      ...
      success(res){
        ...
        wx.request({
          ...
          success(res){
            ...
            temparr1=JSON.stringify(temparr)
            var a=[]
            var a=JSON.stringify(that.data.searcharray)
            var b=[]
            b=JSON.parse(a)
            var list=[]
            var name=b[index].goodsName
            var searcharray=JSON.stringify(that.data.searcharray)
            for(var i=0;i<temparr.length;i++){
              if(temparr[i].goodsName==name){
                list.push(temparr[i])
              }
            }
            var list1=JSON.stringify(list)
            wx.navigateTo({
              url: '/pages/storelist/storelist?info='+list1,
            })
          }   
        })
      }
